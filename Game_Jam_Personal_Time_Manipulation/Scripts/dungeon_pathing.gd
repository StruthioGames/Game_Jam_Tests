...
@export var path_tester_block: PackedScene
...
func generate_dungeon(starting_space):
	rng.randomize()
	generate_main_path(starting_space)
...
func generate_branching_paths():
	...
	while current_maps < TOTAL_MAPS:
		...
		if can_branch_fromm(branch_from):
...
func can_branch_fromm(branch_position):
	branch_position
	return true
...
func print_map():
	...
	for i in range(0, map_nodes.size() -1):
		var path_instance = path_tester_block.instantiate()
		add_child(path_instance)
		var mesh = path_instance.get_child(0).get_child(1)
		var new_material = StandardMaterial3D.new()
		new_material.albedo_color = Color(randf(), randf(), randf())
		mesh.material_override = new_material
		var scaler = mesh.scale
		...
		if direction_moved == Vector3(1,0,0) or direction_moved == Vector3(-1,0,0):
			...
			node_size = scaler.z
		path_instance.global_position = node * node_size
